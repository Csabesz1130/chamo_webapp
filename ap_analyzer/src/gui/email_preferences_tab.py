...
class EmailPreferencesTab:
    ...
    def setup_keyword_section(self):
        ...
        ttk.Button(keyword_entry_frame, text="Hozzáad", 
                  command=self.add_keyword).pack(side='left')
        
        # Kulcsszó lista
        self.keyword_listbox = tk.Listbox(keyword_frame, height=5)
        self.keyword_listbox.pack(fill='x', padx=5, pady=2)
        
        ttk.Button(keyword_frame, text="Törlés", 
                  command=self.remove_selected_keyword).pack(pady=2)
        ...
        ttk.Radiobutton(logic_frame, text="ÉS", variable=self.keyword_logic_var,
                       value='AND', command=self.update_keyword_logic).pack(side='left', padx=5)
        ttk.Radiobutton(logic_frame, text="VAGY", variable=self.keyword_logic_var,
                       value='OR', command=self.update_keyword_logic).pack(side='left', padx=5)
    ...
    def setup_source_section(self):
        ...
        for source, var in self.source_vars.items():
            ttk.Checkbutton(source_frame, text=source, variable=var,
                          command=self.update_sources).pack(anchor='w', padx=5, pady=2)
    ...
    def setup_delivery_section(self):
        ...
        day_combo = ttk.Combobox(day_frame, textvariable=self.delivery_day_var,
                               values=[str(i) for i in range(7)], state='readonly')
        day_combo.pack(side='left', padx=5)
        ttk.Label(day_frame, text=days[int(self.delivery_day_var.get())]).pack(side='left')
        ...
        # Frissítés gomb
        ttk.Button(delivery_frame, text="Frissítés", 
                  command=self.update_delivery_schedule).pack(pady=2)
    ...
    def setup_enable_section(self):
        ...
        ttk.Checkbutton(enable_frame, text="E-mail értesítések engedélyezése",
                       variable=self.enabled_var,
                       command=self.update_enabled).pack(anchor='w')
    ...
    def load_preferences(self):
        ...
        # Logikai operátor
        self.keyword_logic_var.set(self.email_manager.preferences.get('keyword_logic', 'AND'))
